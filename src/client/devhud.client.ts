...
class FormControlBooleanInput extends FormControl {
	...
	tickImageURL: string;
	callbackWhenChanged: ((()=>void) | undefined );
	provided_default_value: boolean;

	constructor(titleText: string, provided_default_value: boolean, callback_when_changed: ((()=>void) | undefined ) = undefined) {
		super(titleText);
		...
		this.provided_default_value = provided_default_value;
		this.callbackWhenChanged = callback_when_changed;
	...
	setup_hooks() {
		this.checked = this.provided_default_value;
		if (this.checked) {
			this.frame.control.container.ImageButton.ImageLabel.Visible = true;
		} else {
			this.frame.control.container.ImageButton.ImageLabel.Visible = false;
...
let editor_controls = [
	["ChunkSize", 16],
	["MaximumChunkLoadingDistance", 128],
	["BlockSize", 4],
	["Seed", 0],
	["Scale", 1],
	["Persistence", 1.15],
...
interface WorldgenGlobals_keyString_valNumberOrBoolean {
	[key: string]: (number | boolean);
}
class EditorHud {

	static editorValues: WorldgenGlobals_keyString_valNumberOrBoolean = {
		"ChunkSize": 16,
		"MaximumChunkLoadingDistance": 128,
		"BlockSize": 4,
		"Seed": 0,
	...
	static init() {
		...
		ScreenGui_Editor.Enabled = true;
		ScreenGui_Editor.background.Visible = true;
		...
		for (let attributepair of editor_controls) {
			...
			let fc: (FormControlNumberInput | FormControlBooleanInput);


			if (type(attributepair[1]) === "number") {
				fc = new FormControlNumberInput(attributepair[0] as string, attributepair[1] as number, ()=>{
					EditorHud.editorValues[(attributepair[0] as string)] = fc.value;
				});
			} else if (type(attributepair[1]) === "boolean") {
				fc = new FormControlBooleanInput(attributepair[0] as string, attributepair[1] as boolean, ()=>{
					EditorHud.editorValues[(attributepair[0] as string)] = fc.value;
				});
			} else {
				error("Uh, no!");
		...
		ScreenGui_Editor.background.submitButton.MouseButton1Click.Connect(()=>{
